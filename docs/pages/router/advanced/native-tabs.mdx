...
You can prevent a tab from being selected natively by setting the `preventSelection` prop on the `NativeTabs.Trigger` component. When `true`, tapping the tab in the tab bar will not change the focused tab.
