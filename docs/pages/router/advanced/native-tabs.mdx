...
By default the native tab bar is lifted above the keyboard on Android. To have the keyboard overlay the tab bar instead, pass `ignoreImeInsets`:

```tsx
<NativeTabs ignoreImeInsets>{/* triggers */}</NativeTabs>
```

> Requires `windowSoftInputMode="adjustResize"` (Expo default) and Android API 30+. Toggling while the keyboard is open has no immediate effect.
