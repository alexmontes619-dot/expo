...
Set `role="destructive"` on a `DropdownMenuItem` to render its text in `MaterialTheme.colorScheme.error`. Caller-supplied `elementColors` still take precedence. Suitable for delete/remove actions.

```tsx
<DropdownMenuItem role="destructive" onClick={onDelete}>
  <DropdownMenuItem.Text>
    <Text>Delete</Text>
  </DropdownMenuItem.Text>
</DropdownMenuItem>
    ...
    <Host matchContents>
      <DropdownMenu expanded={isExpanded} onDismissRequest={() => setIsExpanded(false)}>
        ...
        <DropdownMenu.Items>
          ...
          <DropdownMenuItem role="destructive" onClick={() => setIsExpanded(false)}>
            <DropdownMenuItem.Text>
              <Text>Delete</Text>
            </DropdownMenuItem.Text>
          </DropdownMenuItem>
