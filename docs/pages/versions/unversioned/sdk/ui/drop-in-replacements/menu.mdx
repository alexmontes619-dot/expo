...
- The following props from `@react-native-menu/menu` are not supported: `themeVariant`, `hitSlop`, `isAnchoredToRight`, `subtitle`, `keepsMenuPresented`, `preferredElementSize`, `state: 'mixed'`, and the imperative `ref.show()` API.
  ...
  return (
    <MenuView
      actions={[
        ...
        { id: 'delete', title: 'Delete', image: DELETE, attributes: { destructive: true } },
      ]}
...
Set `shouldOpenOnLongPress` to render as a context menu. On iOS this uses SwiftUI's `ContextMenu` and shows the trigger as a blurred preview; on Android the same controlled `DropdownMenu` opens from the `Pressable`'s `onLongPress` instead of `onPress`.
...
export default function LongPressMenuExample() {
  return (
    <MenuView
      ...
      actions={[
        { id: 'copy', title: 'Copy', image: COPY },
        { id: 'share', title: 'Share', image: SHARE },
      ]}
      onPressAction={e => console.log(e.nativeEvent.event)}>
      <Pressable>
...
`subactions` renders nested actions as a submenu by default. Set `displayInline: true` on the parent to render the children as an inline section instead — useful for grouping. On iOS the parent's `title` becomes the section header; on Android only the divider appears (Material's `DropdownMenu` has no section primitive).
